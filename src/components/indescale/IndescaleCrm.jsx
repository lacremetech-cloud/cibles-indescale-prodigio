...
export default function IndescaleCrm() {
  ...
  async function refresh(pages = 1) {
    ...
    try {
      const res = await fetch('/api/enrich-ville', {
        ...
        body: JSON.stringify({
          ...
          requete_scraping: catObj?.requete_scraping || cat, pages
        })
      })
  ...
  const headerExtra = (
    <div className="banner">
      ...
      <div style={{ marginTop: 8, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        <button className="btn primary" onClick={() => refresh(1)} disabled={busy}>
          {busy ? 'Enrichissement…' : '🔄 Rafraîchir cette ville'}
        </button>
        <button className="btn" onClick={() => refresh(3)} disabled={busy} title="Jusqu'à 60 entreprises (plus d'appels API)">
          + Plus de résultats
        </button>
        <span style={{ fontSize: 12, color: 'var(--muted)' }}>
          N'appelle l'API que sur ce clic — sinon lecture du cache (≈ 20 résultats / clic).
        </span>
      </div>
